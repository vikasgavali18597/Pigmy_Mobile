...
namespace G_Pigmy.Views.BluetoothView
{
    ...
    public partial class BluetoothDevicePage : ContentPage
    {
        ...
        public BluetoothDevicePage()                                                      // constructor (the function that is called when an instance of a class is defined)
        {
            InitializeComponent();
        ...
        private async void FoundBluetoothDevicesListView_ItemTapped(object sender, ItemTappedEventArgs e)   // Function that is run whenever a detected BLE device is selected
        {
            ...
            {
                await Navigation.PushAsync(new BluetoothServicePage(selectedItem));                                   // Navigate to the Services Page to show the services of the selected BLE Device
            }
            else
            {
                try
